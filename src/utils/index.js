...
// Get points along a curved path for better collision detection
const getPathPoints = (fromPos, toPos, numPoints = 10) => {
  const points = [];
  ...
  for (let i = 0; i <= numPoints; i++) {
    ...
    const y = fromPos.y + t * dy + Math.sin(t * Math.PI) * curvature * 0.3;
    points.push({ x, y });
  }
  return points;
};
...
export const isNearConnectionPath = (point, connections, agents, humanOperators, sourceAgentId, threshold = 50) => {
  for (const conn of connections) {
    // Skip connections FROM this agent (we want suggestions in outgoing directions)
    ...
    // Get points along the curved path
    const pathPoints = getPathPoints(fromNode.position, toNode.position, 8);
